1 REM ****** AUTORENNEN ********
2 POKE 650,128:POKE 53281,3:PRINT CHR$(147):POKE53269,0:REM 53281,12
...
2001 PRINT"?":PRINT:PRINT:PRINT:PRINT
2010 PRINTTAB( 9)"?   ??           ? ?"
2020 PRINTTAB( 9)"? ?  ? ??          ? ?"
2030 PRINTTAB( 9)"? ?   ? ?          ? ?    ? ?    "
2040 PRINTTAB( 9)"? ?   ? ?          ? ?          "
...
2380 NEXT
2390 POKE1600,18:POKE1601,1:POKE1602,3:POKE1603,9:POKE164,14:POKE1605,7
...
2750 GOTO2700
2800 PRINT"?????????????? ja ? /  neinN ";:EN=1:RETURN
2900 PRINT"????????????? ja  / ? nein ?";:EN=2:RETURN
3000 IF EN=1 THEN RESTORE:ZIEL=0:GOTO 1
3010 REMPRINT"?":PRINT"??????????????????????????? schade ":
3015 PRINT"?":FORN=1TO189:READ Q:NEXT
